...
function optimizeDashboardLayout() {
    ...
    if (temperatureCard) {
        ...
        if (tempContent) {
            ...
            if (tempContent.offsetHeight > 200 && !temperatureCard.classList.contains('custom-height')) {
    ...
    cards.forEach(card => {
        ...
        if (!card.classList.contains('size-1x1') && 
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const homeButton = document.querySelector('.sidebar button[onclick="showTab(\'home\')"]');
    if (homeButton) {
